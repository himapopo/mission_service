...
var ItemRels = struct {
	...
	GetItemMissions:    "GetItemMissions",
...
type itemR struct {
	GetItemMissions    GetItemMissionSlice    `boil:"GetItemMissions" json:"GetItemMissions" toml:"GetItemMissions" yaml:"GetItemMissions"`
...
func (r *itemR) GetGetItemMissions() GetItemMissionSlice {
	if r == nil {
		return nil
	}
	return r.GetItemMissions
}

func (r *itemR) GetMissionRewardItems() MissionRewardItemSlice {
	if r == nil {
		return nil
	}
...
var (
	itemAllColumns            = []string{"id", "name", "updated_at", "created_at"}
	itemColumnsWithoutDefault = []string{"name"}
	itemColumnsWithDefault    = []string{"id", "updated_at", "created_at"}
...
// GetItemMissions retrieves all the get_item_mission's GetItemMissions with an executor.
func (o *Item) GetItemMissions(mods ...qm.QueryMod) getItemMissionQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"get_item_missions\".\"item_id\"=?", o.ID),
	)
...
func (o *Item) MissionRewardItems(mods ...qm.QueryMod) missionRewardItemQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("\"mission_reward_items\".\"item_id\"=?", o.ID),
	)
...
func (o *Item) UserItems(mods ...qm.QueryMod) userItemQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"user_items\".\"item_id\"=?", o.ID),
	)

	return UserItems(queryMods...)
}

// LoadGetItemMissions allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (itemL) LoadGetItemMissions(ctx context.Context, e boil.ContextExecutor, singular bool, maybeItem interface{}, mods queries.Applicator) error {
	var slice []*Item
	var object *Item
	...
	query := NewQuery(
		...
		qm.WhereIn(`get_item_missions.item_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load get_item_missions")
	}

	var resultSlice []*GetItemMission
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice get_item_missions")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on get_item_missions")
	}
	...
	if singular {
		object.R.GetItemMissions = resultSlice
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.ItemID {
				local.R.GetItemMissions = append(local.R.GetItemMissions, foreign)
				if foreign.R == nil {
					foreign.R = &getItemMissionR{}
				}
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (itemL) LoadMissionRewardItems(ctx context.Context, e boil.ContextExecutor, singular bool, maybeItem interface{}, mods queries.Applicator) error {
	var slice []*Item
	var object *Item
	...
	query := NewQuery(
		...
		qm.WhereIn(`mission_reward_items.item_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (itemL) LoadUserItems(ctx context.Context, e boil.ContextExecutor, singular bool, maybeItem interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`user_items`),
		qm.WhereIn(`user_items.item_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
// AddGetItemMissions adds the given related objects to the existing relationships
// of the item, optionally inserting them as new records.
// Appends related to o.R.GetItemMissions.
// Sets related.R.Item appropriately.
func (o *Item) AddGetItemMissions(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*GetItemMission) error {
	var err error
	...
	if o.R == nil {
		o.R = &itemR{
			GetItemMissions: related,
		}
	} else {
		o.R.GetItemMissions = append(o.R.GetItemMissions, related...)
	}

	for _, rel := range related {
		if rel.R == nil {
			rel.R = &getItemMissionR{
				Item: o,
			}
		} else {
			rel.R.Item = o
...
// Sets related.R.Item appropriately.
func (o *Item) AddMissionRewardItems(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*MissionRewardItem) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.ItemID = o.ID
